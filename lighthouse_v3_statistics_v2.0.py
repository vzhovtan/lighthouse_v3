...
def task(env, action, caseid="None", platform="None", technology="None", feedback="None"):
    ...
    if backend_action == "get_admin_status":
        ...
        result.append(data)
    elif backend_action == "get_stats_as_csv":
        data = get_stats_as_csv (dbaas)
        result.append(data)
    else:
...
def get_stats_as_csv (dbaas):
    today = datetime.date.today()
    file_name_csv = "lighthouse_stats_file_" + str(today) + ".csv"
    with open (file_name_csv, "w") as file:
        fieldnames = ['caseid', 'platform', 'technology', 'feedback']
        writer = csv.DictWriter(file, fieldnames=fieldnames)
        writer.writeheader()
        for item in dbaas["lighthouse_statistics"].find():
            writer.writerow({'caseid': item['caseid'], 'platform': item['platform'], 'technology': item['technology'], 'feedback' : item['feedback']})

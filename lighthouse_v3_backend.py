...
def task(env, action, platform="None", technology="None", commands="None", links="None", owner="None"):
    """
    ...
    if backend_action == "get_admin_status":
        ...
        update = get_technology_formatted(platform, technology, dbaas)
        result.append(update)
...
def get_platform_list(dbaas):
    # get platform list from MongoDB
    platform_list = []
    for item in dbaas["lighthouse"].find():
        if "_final" in item["_id"]:
            platf_info = item["_id"].replace("_", " ").split()
            if platf_info[0] not in platform_list:
                platform_list.append(platf_info[0].lower())
                
    platform_list.sort()
    return platform_list

def get_technology_list(platform, dbaas):
    # get technology list from MongoDB for specific platform 
    # platform is a key and version has to be final, i.e. approved 
    technology_list = []
    for doc in dbaas["lighthouse"].find():
        if "_final" in doc["_id"]:
            tech_info = doc["_id"].replace("_", " ").split()
            if tech_info[0] == platform.lower() and tech_info[1] not in technology_list:
                technology_list.append(tech_info[1].lower())
    
    technology_list.sort()
    return technology_list

def get_technology (platform, technology, dbaas):
    ...
    tech_result = []
    tech_id = platform + "_" + technology + "_final"
    doc = dbaas["lighthouse"].find_one({"_id" : tech_id})
    tech_result.append(doc["commands"])
    tech_result.append(doc["links"])

    return tech_result

def get_technology_formatted(platform, technology, dbaas):
    # get commands and links from MongoDB for specific platform and technology
    # platform and technology are keys and version has to be final, i.e. approved 
    # format the output as html for BJB
    result_formatted = ""
    ...
    #result_formatted += "<div style='text-align:right'><span class='thumbs-up' style='color:green' title='Like'><img src='https://i.imgur.com/OFZtTLn.jpg' alt=''></span>&emsp;<span class='thumbs-down' style='color:red' title='Not so much'><img src='https://i.imgur.com/rdB66kQ.jpg' alt=''></span>&emsp;</div>"
    #result_formatted += "<div style='text-align:right'><span class='thumbs-up' style='color:green' title='Like'><img src='https://i.imgur.com/OFZtTLn.jpg' alt=''><br><br></span></div>"
    #result_formatted += "<div style='text-align:right'><span class='thumbs-down' style='color:red' title='Not so much'><img src='https://i.imgur.com/rdB66kQ.jpg' alt=''></span></div>"
    result_formatted += "<b><font size='4'><div id='LighthouseContentTitle' style='text-align:center'><h3>Lighthouse : " + platform.upper() + " - " + technology.upper() + "</h3></div></b></font><div id='LighthouseContentBody'>"
    ...
    for item in cmd:
        if "//" in item:
            ...
            newItem = "<span>" + command + "</span>" + "<span style='color:green; float:right'>" + comment + "</span>"
